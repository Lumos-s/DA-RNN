...
def init_hidden(x, hidden_size: int):
    ...
    """
    return torch.zeros(1, x.size(0), hidden_size, device=x.device)
...
class Encoder(nn.Module):
    ...
    def __init__(self, input_size: int, hidden_size: int, T: int):
        ...
        self.attn_linear = nn.Sequential(
            nn.Linear(in_features=2 * hidden_size + T, out_features=T),
            nn.Tanh(),
    ...
    def forward(self, input_data):
        ...
        input_weighted = torch.zeros(
            input_data.size(0), self.T, self.input_size, device=device
        )
        input_encoded = torch.zeros(
            input_data.size(0), self.T, self.hidden_size, device=device
        )
        ...
        for t in range(self.T):
            ...
            attn_weights = tf.softmax(
                x.view(-1, self.input_size), dim=1
            )  # (batch_size, input_size)
            # Eqn. 10: LSTM
            weighted_input = torch.mul(
                attn_weights, input_data[:, t, :]
            )  # (batch_size, input_size)
...
class Decoder(nn.Module):
    ...
    def __init__(
        self, encoder_hidden_size: int, decoder_hidden_size: int, T: int, out_feats=1
    ):
    ...
    def forward(self, input_encoded, y_history):
        ...
        for t in range(self.T - 1):
            ...
            # Eqn. 15
            y_tilde = self.fc(
                torch.cat((context, y_history[:, t]), dim=1)
            )  # (batch_size, out_size)
            # Eqn. 16: LSTM
            self.lstm_layer.flatten_parameters()
